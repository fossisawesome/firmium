...
pub fn spawn_finish_watcher(sessions: SessionMap, bus: EventBus, player_id: PlayerId) {
    tokio::spawn(async move {
        ...
        loop {
            ...
            let (finished, position) = {
                ...
                match s.get(&player_id) {
                    ...
                    Some(session) => {
                        ...
                        let position = if tick.is_multiple_of(3) && !loading && session.playback_start_time.lock().is_some() {
                            let pos = *session.accumulated_time.lock()
                                + session.playback_start_time.lock()
                                    .map(|t| t.elapsed().as_secs_f64())
                                    .unwrap_or(0.0);
                            Some((pos, *session.duration.lock()))
                        } else {
                            None
                        };
                        (finished, position)
            ...
            if let Some((pos, duration)) = position {
                bus.emit(BackendEvent::PlaybackPosition {
            ...
            if finished {
                ...
                bus.emit(BackendEvent::PlaybackFinished { player_id: player_id.clone() });
                break;
            }
        }
    });
