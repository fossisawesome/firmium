...
use crate::commands::local_library::{find_local_match_internal, get_local_track_path};
...
pub(crate) async fn stream_url_for(song: &Song, app_state: &AppState) -> Result<String, String> {
    if song.id.starts_with("local:") {
        let path = get_local_track_path(app_state, &song.id)?;
        return Ok(format!("file://{path}"));
    }
    if let Some(path) = find_local_match_internal(app_state, &song.title, &song.artist, &song.album) {
        return Ok(format!("file://{path}"));
    }
...
pub(crate) fn schedule_save_play_queue(
    ...
    let handle = tokio::spawn(async move {
        ...
        fire_save_play_queue(app_state, ids, current_id, pos_ms);
    });
...
pub(crate) async fn play_at(
    ...
    if let (Some(preloaded_pid), Some(preloaded_tid)) = (&preloaded_id, &preloaded_track_id) {
        if preloaded_tid == &song.id {
            ...
            schedule_save_play_queue(app_state, queue_state, Some(pid), audio_player);
            return Ok(());
        }
    }
    ...
    let stream_url = stream_url_for(&song, app_state).await?;
...
/// Restores persisted playback settings on startup. Does not emit
/// queue-state-changed — startup avoids the event race.
...
pub fn set_auto_continue(state: &QueueState, enabled: bool) {
    state.inner.lock().auto_continue = enabled;
}

pub fn set_replay_gain_enabled(state: &QueueState, audio_player: &AudioPlayer, enabled: bool) {
...
pub fn set_repeat_mode(bus: &EventBus, state: &QueueState, repeat_one: bool, repeat_all: bool) {
    {
        let mut inner = state.inner.lock();
...
pub fn toggle_shuffle(bus: &EventBus, state: &QueueState) {
    {
        let mut inner = state.inner.lock();
...
pub fn set_crossfade_settings(bus: &EventBus, state: &QueueState, enabled: bool, duration_secs: f32) {
    {
        let mut inner = state.inner.lock();
...
pub fn set_gapless_enabled(bus: &EventBus, state: &QueueState, enabled: bool) {
...
pub async fn set_queue(
    ...
    play_at(&queue_state, &app_state, &audio_player, start_idx).await
}
...
pub async fn shuffle_and_play(
    ...
    play_at(&queue_state, &app_state, &audio_player, 0).await
}
...
pub async fn play_queue_index(
    ...
    play_at(&queue_state, &app_state, &audio_player, idx).await
}
...
pub async fn queue_next(
    ...
    play_at(&queue_state, &app_state, &audio_player, next_idx).await
}
...
pub async fn queue_prev(
    ...
    if queue_idx > 0 {
        play_at(&queue_state, &app_state, &audio_player, (queue_idx - 1) as usize).await
    } else {
        Ok(())
    }
}
...
pub async fn toggle_play(
    ...
    if let Some(ref pid) = current_player_id {
        match audio_player.get_state(pid) {
            Ok(crate::PlaybackState::Playing) => {
                ...
                let song_id = queue_state.inner.lock().queue.get(queue_idx as usize).map(|s| s.id.clone()).unwrap_or_default();
                let pos_ms = (audio_player.get_current_position(pid).unwrap_or(0.0) * 1000.0) as i64;
                fire_report_playback(Arc::clone(&app_state), song_id, pos_ms, "paused".into());
                return Ok(());
            }
            Ok(crate::PlaybackState::Paused) => {
                audio_player.resume(pid)?;
                let song_id = queue_state.inner.lock().queue.get(queue_idx as usize).map(|s| s.id.clone()).unwrap_or_default();
                fire_report_playback(Arc::clone(&app_state), song_id, 0, "playing".into());
                return Ok(());
            }
            _ => {}
        }
    ...
    if queue_idx >= 0 {
        play_at(&queue_state, &app_state, &audio_player, queue_idx as usize).await
    } else {
        Ok(())
    }
}
...
pub fn seek_queue(queue_state: &QueueState, audio_player: &AudioPlayer, position: f64) -> Result<(), String> {
...
pub fn set_queue_volume(bus: &EventBus, queue_state: &QueueState, audio_player: &AudioPlayer, volume: f32) {
    ...
    emit_queue_state(bus, queue_state);
}
