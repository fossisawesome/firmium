        ...
        match message {
            ...
            Message::SetCrossfadeEnabled(on) => {
                ...
                if on {
                    if self.gapless_enabled {
                        ...
                        firmium_backend::commands::queue::set_gapless_enabled(&self.backend.bus, &self.backend.queue_state, false);
                    }
                ...
                firmium_backend::commands::queue::set_crossfade_settings(&self.backend.bus, &self.backend.queue_state, on, self.crossfade_duration);
                Task::none()
            }
            Message::SetCrossfadeDuration(secs) => {
                self.crossfade_duration = secs;
                firmium_backend::commands::queue::set_crossfade_settings(&self.backend.bus, &self.backend.queue_state, self.crossfade_enabled, secs);
                Task::none()
            }
            Message::SetGapless(on) => {
                ...
                if on && self.crossfade_enabled {
                    ...
                    firmium_backend::commands::queue::set_crossfade_settings(&self.backend.bus, &self.backend.queue_state, false, self.crossfade_duration);
                }
                firmium_backend::commands::queue::set_gapless_enabled(&self.backend.bus, &self.backend.queue_state, on);
                Task::none()
            }
            Message::SetReplayGain(on) => {
                self.replay_gain_enabled = on;
                firmium_backend::commands::queue::set_replay_gain_enabled(&self.backend.queue_state, &self.backend.audio_player, on);
                Task::none()
            }
            Message::SetAutoContinue(on) => {
                self.auto_continue = on;
                firmium_backend::commands::queue::set_auto_continue(&self.backend.queue_state, on);
                Task::none()
            }
            ...
            Message::SetLastfmEnabled(on) => {
                ...
                if !on {
                    let _ = firmium_backend::commands::credentials::delete_password(Some("firmium-desktop"), "lastfm_key");
                    let _ = firmium_backend::commands::credentials::delete_password(Some("firmium-desktop"), "lastfm_secret");
            ...
            Message::SetLastfmKey(key) => {
                ...
                let _ = firmium_backend::commands::credentials::save_password(Some("firmium-desktop"), "lastfm_key", &key);
                Task::none()
            }
            Message::SetLastfmSecret(secret) => {
                self.lastfm_secret = secret.clone();
                let _ = firmium_backend::commands::credentials::save_password(Some("firmium-desktop"), "lastfm_secret", &secret);
                Task::none()
            }
            Message::SetListenbrainzEnabled(on) => {
                self.listenbrainz_enabled = on;
                if !on {
                    let _ = firmium_backend::commands::credentials::delete_password(Some("firmium-desktop"), "listenbrainz_token");
            ...
            Message::SetListenbrainzToken(token) => {
                ...
                let _ = firmium_backend::commands::credentials::save_password(Some("firmium-desktop"), "listenbrainz_token", &token);
                Task::none()
            }
            ...
            Message::WipeCoverCache => {
                let _ = firmium_backend::commands::cover_cache::clear_cover_cache();
            ...
            Message::DeleteSettings => {
                ...
                firmium_backend::commands::queue::set_gapless_enabled(&self.backend.bus, &self.backend.queue_state, self.gapless_enabled);
                firmium_backend::commands::queue::set_replay_gain_enabled(&self.backend.queue_state, &self.backend.audio_player, self.replay_gain_enabled);
                firmium_backend::commands::queue::set_auto_continue(&self.backend.queue_state, self.auto_continue);
                firmium_backend::commands::queue::set_crossfade_settings(&self.backend.bus, &self.backend.queue_state, self.crossfade_enabled, self.crossfade_duration);
                self.save_config();
                Task::none()
            }
            _ => unreachable!(),
