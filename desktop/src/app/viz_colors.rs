...
impl App {
    ...
    pub(crate) fn maybe_fetch_viz_colors(&mut self) -> Task<Message> {
        ...
        let Ok(url) = firmium_backend::commands::subsonic::build_cover_url(&self.backend.app_state, &cover_id, 300) else {
            return Task::none();
        };
        let track_id = song.id.clone();
        Task::perform(
            firmium_backend::commands::cover_colors::extract_cover_colors(cover_id, url),
