...
impl AudioPlayer {
    ...
    fn start_session(self_arc: &Arc<Self>, stream_url: &str, track_id: String, start_paused: bool, replay_gain_db: Option<f32>) -> Result<PlayerId, String> {
        ...
        tauri::async_runtime::spawn(async move {
            let decode_result = tauri::async_runtime::spawn_blocking(move || Self::fetch_and_open(http_client, &stream_url)).await;
                    ...
                    if start_paused {
                        ...
                        *session.accumulated_time.lock() = 0.0;
                        ...
                        let _ = app_handle.emit("playback-state-changed", serde_json::json!({
                            ...
                            "state": "playing",
                            "audioInfo": { "sampleRate": native_rate, "channels": native_channels }
                        }));

                        session::spawn_finish_watcher(Arc::clone(&sessions), app_handle.clone(), player_id_clone.clone());
                    }
                }
                Ok(Err(e)) => {
                    ...
                    if !start_paused {
                        let _ = app_handle.emit("playback-state-changed", serde_json::json!({"playerId": player_id_clone, "state": "stopped"}));
                    }
                }
                Err(join_err) => {
    ...
    pub fn pause(&self, player_id: &str) -> Result<(), String> {
        ...
        let _ = self.app_handle.emit("playback-state-changed", serde_json::json!({"playerId": player_id, "state": "paused"}));
        Ok(())
    }
    ...
    pub fn resume(&self, player_id: &str) -> Result<(), String> {
        ...
        let _ = self.app_handle.emit("playback-state-changed", serde_json::json!({"playerId": player_id, "state": "playing"}));

        if needs_watcher {
            session::spawn_finish_watcher(Arc::clone(&self.sessions), self.app_handle.clone(), player_id.to_string());
        }
        Ok(())
    }
