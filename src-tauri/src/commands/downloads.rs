...
#[tauri::command]
pub async fn download_album(app: AppHandle, state: State<'_, Arc<AppState>>, album_id: String, format: String) -> Result<(), String> {
    let album_tracks = get_album_tracks(app.clone(), state, album_id).await?;
    for track in album_tracks.tracks {
        let track_state = app.state::<Arc<AppState>>();
        download_track(
            app.clone(),
            track_state,
            track.id,
