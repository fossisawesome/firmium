...
pub fn local_library_dir(app: &AppHandle) -> Result<PathBuf, String> {
    let dir = app.path().audio_dir().map_err(|e| e.to_string())?.join("Firmium");
...
/// Cached scan results, keyed for fast lookup by the `get_local_*` commands.
...
fn scan(app: &AppHandle) -> Result<LocalLibraryCache, String> {
    let root = local_library_dir(app)?;
    let mut raw = Vec::new();
    walk(&root, &mut raw);
...
#[tauri::command]
pub async fn import_local_files(app: AppHandle, state: State<'_, Arc<AppState>>, paths: Vec<String>) -> Result<usize, String> {
    let root = local_library_dir(&app)?;
...
#[tauri::command]
pub fn get_local_albums(app: AppHandle, state: State<'_, Arc<AppState>>) -> Result<Vec<Album>, String> {
    ensure_scanned(&app, &state)?;
    Ok(state.local_library.read().as_ref().unwrap().albums.clone())
}

#[tauri::command]
pub fn get_local_artists(app: AppHandle, state: State<'_, Arc<AppState>>) -> Result<Vec<Artist>, String> {
    ensure_scanned(&app, &state)?;
...
#[tauri::command]
pub fn get_local_album_tracks(app: AppHandle, state: State<'_, Arc<AppState>>, id: String) -> Result<LocalAlbumTracks, String> {
    ensure_scanned(&app, &state)?;
...
#[tauri::command]
pub fn get_local_album_track_keys(app: AppHandle, state: State<'_, Arc<AppState>>, album_artist: String, album: String) -> Result<Vec<LocalTrackKey>, String> {
    ensure_scanned(&app, &state)?;
...
#[tauri::command]
pub fn get_local_artist_details(app: AppHandle, state: State<'_, Arc<AppState>>, id: String) -> Result<LocalArtistDetails, String> {
    ensure_scanned(&app, &state)?;
    let cache = state.local_library.read();
    let cache = cache.as_ref().unwrap();
...
#[tauri::command]
pub fn get_local_track_path(app: AppHandle, state: State<'_, Arc<AppState>>, id: String) -> Result<String, String> {
    ensure_scanned(&app, &state)?;
    let cache = state.local_library.read();
...
/// `app_cache_dir/local_covers/`, and returns the cached file path (mirrors `get_cover_art`).
#[tauri::command]
pub fn get_local_cover_art(app: AppHandle, state: State<'_, Arc<AppState>>, id: String) -> Result<String, String> {
    ensure_scanned(&app, &state)?;
    let path = {
        let cache = state.local_library.read();
    ...
    let dir = app.path().app_cache_dir().map_err(|e| e.to_string())?.join("local_covers");
    std::fs::create_dir_all(&dir).map_err(|e| e.to_string())?;
...
#[tauri::command]
pub fn search_local(app: AppHandle, state: State<'_, Arc<AppState>>, query: String) -> Result<LocalSearchResult, String> {
    ensure_scanned(&app, &state)?;
...
#[tauri::command]
pub fn get_local_recent_albums(app: AppHandle, state: State<'_, Arc<AppState>>, size: u32) -> Result<Vec<Album>, String> {
    ensure_scanned(&app, &state)?;
    let cache = state.local_library.read();
    let cache = cache.as_ref().unwrap();
...
#[tauri::command]
pub fn get_local_random_albums(app: AppHandle, state: State<'_, Arc<AppState>>, size: u32) -> Result<Vec<Album>, String> {
...
#[tauri::command]
pub fn get_local_genres_list(app: AppHandle, state: State<'_, Arc<AppState>>) -> Result<Vec<LocalGenre>, String> {
    ensure_scanned(&app, &state)?;
    let cache = state.local_library.read();
    let cache = cache.as_ref().unwrap();
