...
/// Managed Tauri state holding the play-history connection.
pub struct PlayHistory {
    conn: Mutex<Connection>,
}

/// Fire-and-forget play-history write, mirroring `fire_scrobble` /
/// `fire_listenbrainz_listen`. No-op when the DB failed to init; errors are
/// logged only so a DB problem never blocks playback.
pub(crate) fn fire_record_play(app: &AppHandle, song: &Song, duration_played: i64) {
    let Some(history) = app.try_state::<PlayHistory>() else { return };
...
fn db_path(app: &AppHandle) -> Result<PathBuf, String> {
    // app_data_dir (not cache) so history survives a cover-cache clear.
    let dir = app.path().app_data_dir().map_err(|e| e.to_string())?;
    std::fs::create_dir_all(&dir).map_err(|e| e.to_string())?;
    Ok(dir.join("play_history.db"))
}
